package com.example.eatmeserver.common.util.login_utils;
...
public class TokenUtils {
    ...
    private static final String jwtSecretKey = "eatmeSecretKey";

    /**
     * 사용자 정보를 기반으로 토큰을 생성하여 반환 해주는 메서드
     *
     * @param param LoginParam : 사용자 정보
     * @return String : 토큰
     */
    public static String generateJwtToken(LoginParam param) {
        // 사용자 시퀀스를 기준으로 JWT 토큰을 발급하여 반환해줍니다.
        JwtBuilder builder = Jwts.builder()
                .setHeader(createHeader())                              // Header 구성
                .setClaims(createClaims(param))                       // Payload - Claims 구성
                .setSubject(String.valueOf(param.getUserSq()))        // Payload - Subject 구성
     ...
     */
    private static Map<String, Object> createClaims(LoginParam param) {
        // 공개 클레임에 사용자의 이름과 이메일을 설정하여 정보를 조회할 수 있다.
        Map<String, Object> claims = new HashMap<>();

        log.info("userId :" + param.getUserId());
        log.info("userNm :" + param.getUserNm());

        claims.put("userId", param.getUserId());
        claims.put("userNm", param.getUserNm());
        return claims;
    }
